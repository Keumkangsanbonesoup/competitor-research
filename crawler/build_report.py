...
- util: 이 프로모션의 **특징 중 B마트가 참고할 만한 포인트를 제안**하는 톤으로 2문장(합쳐 최대 120자). **단정·지시형 금지**('B마트는 ~해야 한다/~해야 합니다' 같은 표현 쓰지 마). 대신 '~점은 참고할 만함', '~방식은 B마트 □□에 응용 여지가 있음', '~는 벤치마크 포인트' 같은 **제안·관찰형**으로. 추측성 단정 금지, 텍스트에서 드러난 특징에 근거해서만.
